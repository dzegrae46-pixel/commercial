...
function FinanceWorkspacePage({
  ...
  useEffect(() => {
    ...
    timeline.label("start")
      ...
      .add(workspace.querySelectorAll("[data-finance-card]"), { opacity: [0, 1], y: [20, 0], delay: 70 }, "start+=90")
      .add(workspace.querySelectorAll("[data-finance-stat]"), { opacity: [0, 1], y: [10, 0], delay: 70 }, "start+=260");
...
export default function WorkspaceApp() {
  ...
  const printDocument = (context: DocumentContext) => {
    void openDeliveryNotePdf(company, context).catch((error) => {
      notify(error instanceof Error ? error.message : "Impossible de préparer le bon de livraison.");
    });
  };
  ...
  return (
    <div className={`app-shell ${activeDocumentEditor ? "document-fullscreen-mode" : ""}`.trim()}>
      ...
      {documentDetails && <DocumentDetailsModal document={documentDetails.document} onClose={() => setDocumentDetails(null)} onPrint={() => printDocument(documentDetails)} />}
